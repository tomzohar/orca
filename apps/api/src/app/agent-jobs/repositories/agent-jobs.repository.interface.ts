...
export interface IAgentJobsRepository {
    create(data: { prompt: string; assignee?: string; type?: AgentType }): Promise<AgentJobEntity>;
    ...
    addArtifact(jobId: number, artifact: { filename: string; content: string }): Promise<AgentJobEntity>;
}
