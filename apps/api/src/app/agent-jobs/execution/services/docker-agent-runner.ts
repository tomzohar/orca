...
import { LogMatcher, LogMatcherContext } from '../matchers/log-matcher.interface';
...
export class DockerAgentRunner implements IAgentRunner {
    ...
    async run(job: AgentJobEntity): Promise<void> {
        ...
        try {
            ...
            const watcher = fs.watch(projectRoot, { recursive: true }, async (eventType, filename) => {
                if (!filename || filename.includes('.git') || filename.includes('node_modules') || filename.includes('.nx') || filename === '.env') return;
                ...
                try {
                    if (fs.existsSync(fullPath)) {
                        ...
                        if (stat.isFile()) {
                            ...
                            this.logger.debug(`[File Watcher] Detected change in ${filename}`);
            ...
            dockerProcess.stdout.on('data', async (data) => {
                ...
                if (message) {
                    ...
                    const context: LogMatcherContext = {
                        job,
                        log: async (msg) => { this.logger.log(msg); },
                        updateStatus: async (status) => await this.updateStatus(job.id, status as AgentJobStatus),
                        setIsWaitingForUser: (val) => { isWaitingForUser = val; }
                    };
            ...
            await this.addLog(job.id, `System Error: ${error instanceof Error ? error.message : String(error)}`);
            await this.updateStatus(job.id, AgentJobStatus.FAILED);
        }
    }
    ...
    private async addArtifact(jobId: number, filename: string, content: string) {
        const updatedJob = await this.repository.addArtifact(jobId, { filename, content });
        ...
        if (artifact) {
            this.eventEmitter.emit(
                ...
                new JobArtifactAddedEvent(jobId, artifact.id, filename, `db://${artifact.id}`)
            );
        }
    }
}
