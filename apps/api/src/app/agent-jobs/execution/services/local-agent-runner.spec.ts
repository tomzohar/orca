...
import { AGENT_JOBS_REPOSITORY, IAgentJobsRepository } from '../../domain/interfaces/agent-jobs.repository.interface';
import { ARTIFACT_STORAGE, IArtifactStorage } from '../../domain/interfaces/artifact-storage.interface';
...
describe('LocalAgentRunner', () => {
    ...
    beforeEach(async () => {
        const mockRepo = {
            ...
            addLog: jest.fn().mockResolvedValue({ ...mockJob, logs: [{ id: 1, message: 'test log', timestamp: new Date() }] }),
    ...
    it('should run a job successfully', async () => {
        ...
        expect(eventEmitter.emit).toHaveBeenCalledWith('agent-job.status-changed', expect.anything());
        expect(eventEmitter.emit).toHaveBeenCalledWith('agent-job.log-added', expect.anything());
    });
    ...
    it('should handle errors during execution', async () => {
        ...
        expect(repository.update).toHaveBeenCalledWith(1, { status: AgentJobStatus.FAILED });
        expect(eventEmitter.emit).toHaveBeenCalledWith('agent-job.status-changed', expect.objectContaining({
