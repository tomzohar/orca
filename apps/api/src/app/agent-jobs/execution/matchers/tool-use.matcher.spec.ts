...
describe('ToolUseMatcher', () => {
  ...
  it('should match the tool use pattern for AskUserQuestion', () => {
    const message =
      '[Agent] 🛠️  Tool Use (AskUserQuestion): {"question": "Are you sure?"}';
  ...
  it('should match other tool uses', () => {
    const message =
      '[Agent] 🛠️  Tool Use (save_artifact): {"filename": "test.txt"}';
    const match = matcher.match(message);
    expect(match).toBeTruthy();
  ...
  it('should handle AskUserQuestion by updating status and context', async () => {
    ...
    const match = [
      ...
      'AskUserQuestion',
      'args',
    ] as any as RegExpMatchArray;

    await matcher.handle(mockContext, match);

    expect(mockContext.updateStatus).toHaveBeenCalledWith(
      AgentJobStatus.WAITING_FOR_USER,
    );
